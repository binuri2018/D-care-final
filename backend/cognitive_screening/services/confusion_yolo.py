...
Place weights at one of:
  ...
  - backend/cognitive_screening/ml_artifacts/confusion_yolo/best.pt
...
def _repo_root() -> Path:
    # cognitive_screening/services/confusion_yolo.py → repo root
...
def default_model_path() -> Path | None:
    ...
    candidates = [
        ...
        Path(__file__).resolve().parents[1] / "ml_artifacts" / "confusion_yolo" / "best.pt",
    ]
    for p in candidates:
        if p.is_file():
            return p
