...
export default function MmseTest() {
  ...
  useEffect(() => {
    if (!patient || !patient.fullName) {
      nav("/screening/patient", { replace: true });
    }
  }, [patient, nav]);
  ...
  useEffect(() => {
    if (phase === "idle" || sessionStartRef.current === 0) {
      return undefined;
    }
    const id = setInterval(() => setElapsedTick((n) => n + 1), 200);
  ...
  }, [phase, step, elapsedTick]);

  return (
    <div className="app-shell">
      <div className="row" style={{ justifyContent: "space-between" }}>
        <Link to="/screening">← Home</Link>
        <div className="timer">
      ...
      {patient && (
        <div className="card" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          ...
          <Link className="btn btn-ghost" to="/screening/patient">
            Edit patient info
          </Link>
        </div>
      )}
